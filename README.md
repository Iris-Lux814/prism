...
**状态：** v0.2.0 — 单用户部署稳定运行中。v1.0 前公开 API 可能调整。

---

## 这是什么

imprint-memory 不是把整段聊天塞回模型上下文的"聊天记录总结器"。它把原始消息保存为本地、可核对的 source；把可重建的连续性状态、摘要和检索索引放到 derived。模型只收到当轮真正需要的极小连续包或带来源的证据包。
